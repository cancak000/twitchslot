...
def spin_individual_reels(root, slots, username="ゲスト", force_level=0, reel_symbols=None, sounds=None, sound_enabled=True, username_queue=None, spin_lock=None):
    try:
        ...
        root.after(6000, lambda: hide_window() if username_queue.empty() else None)
        spin_lock.release()
...
def start_spin_with_user(root, slots, username, force_level=0, reel_symbols=None, sounds=None, sound_enabled=True, username_queue=None, spin_lock=None):
    ...
    if not acquired:
        ...
        return
    t = threading.Thread(target=spin_individual_reels, args=(root, slots, username, force_level, reel_symbols, sounds, sound_enabled, username_queue, spin_lock))
    t.start()
    # アニメーションなど全て終了した後
    spin_lock.release()